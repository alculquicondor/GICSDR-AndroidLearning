...
public class MP_Persona {
	public static String sqlInsert(Persona persona) {
		return "INSERT INTO persona (nombres, appaterno, apmaterno) VALUES ('"
				+ persona.getNombres() + "', '" + persona.getAppaterno()
	...
	public static Contactos loadFromDb(Context context) {
		...
		if (db != null) {
			...
			Cursor cursor = db
					.rawQuery(MP_Persona.sqlGetAll(), new String[] {});
			if (cursor.moveToFirst()) {
				do {
					...
					Persona p = new Persona(_id, _nombres, _appaterno,
							_apmaterno);
					c.agregarPersona(p);
	...
	public static Contactos getById(Context context, Integer id) {
		...
		if (db != null) {
			...
			Cursor cursor = db.rawQuery(MP_Persona.sqlGetById(id),
					new String[] {});
			if (cursor.moveToFirst()) {
				do {
					Integer _id = cursor.getInt(0);
